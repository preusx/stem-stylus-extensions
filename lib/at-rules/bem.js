...
var plugin = function(style) {
  ...
  var bemRuleBlock = function(atrule) {
    ...
    bemBlockScope.push(segments.concat(atrule.segments));
  ...
  var bemRuleNested = function(atrule) {
    var currentBlock = bemBlockScope[bemBlockScope.length - 1];
    var segments = [
        new n.Literal(' '),
      ].concat(currentBlock);
  ...
  style.options.extensions.atRuleType.register('bn', bemRuleNested);
  // style.options.extensions.atRuleType.register('block-nested', bemRuleBlock);
  // style.options.extensions.atRuleType.register('component-nested', bemRuleBlock);
