var utils = require('stylus/lib/utils.js');

var plugin = function(style){
    var type;

    style.define('bgg', function() {
        var n = style.nodes;
        var prop = new n.Property([new n.Literal('background')]);
        prop.expr = new n.Expression();
        prop.expr.nodes = Array.prototype.slice.call(arguments);

        return prop;
        // this.mixin();
    });

    style.define('props-test', function() {
        var n = style.nodes;
        var b = this.currentBlock;
        var caller = b.nodes[b.index];

        console.log(caller.block.nodes);

        // if(caller.block) {
        //     var m = new n.Media([new n.QueryList('(min-width: 40px)')]);

        //     m.block = caller.block;
        //     console.log(m);

        //     return m;
        // }

        return caller.block;
        // console.log(this.stack.currentFrame.block);
        // console.log(this.closestBlock);
        // console.log(arguments);

        // this.mixin();
    });

    (type = function(expr) {
        expr = utils.unwrap(expr);

        if(expr.nodes.length > 1) return 'list';
        if(typeof expr.first.vals == 'object') return 'hash';

        return expr.first.nodeName;
    }).raw = true;

    var nodeTypes = [
            'string',
            'list',
            'hash',
            'ident',
            'literal',
            'block',
            'function',
            'boolean',
            'rgba',
            'null',
            'unit',
            'hsla',
        ];

    style.define('type', type);

    for(var i = nodeTypes.length - 1; i >= 0; i--) {
        style.define('is_' + nodeTypes[i], (function(nodeType) {
            var fnc;

            (fnc = function(expr) {
                if(type(expr) == nodeType) return true;
            }).raw = true;

            return fnc;
        })(nodeTypes[i]));
    }


    style.define('is_color', (function() {
        var fnc;

        (fnc = function(expr) {
            var tp = type(expr);

            if(tp == 'rgba' || tp == 'hsla') return true;
        }).raw = true;

        return fnc;
    })());
};

module.exports = {
  plugin: plugin,
}