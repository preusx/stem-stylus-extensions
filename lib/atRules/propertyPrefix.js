...
var plugin = function(style){
  ...
  var hashToObject = function(hash) {
    var object = {};
    ...
    if(hash.nodeName == 'object') {
      for(var i in hash.vals) {
        ...
        var val = utils.unwrap(hash.vals[i]).nodes;

        for(var j = 0; j < val.length; j++) {
          val[j] = utils.unwrap(val[j]);
  ...
  style.define('-settings', function(hash) {
    ...
    // console.log(JSON.stringify(hashToObject(hash), null, 2));
