...
/**
 * Traslates stylus hash to JS object.
 *
 * @param  {hash} hash  - Stylus hash
 * @return {object}     - JS object
 */
var hashToObject = module.exports.hashToObject = function(hash) {
  var object = {};

  if(hash.nodeName == 'object') {
    for(var i in hash.vals) {
      if(!hash.vals.hasOwnProperty(i)) continue;

      var val = su.unwrap(hash.vals[i]).nodes;

      for(var j = 0; j < val.length; j++) {
        val[j] = su.unwrap(val[j]);

        if(val[j].nodeName == 'object') {
          val[j] = hashToObject(val[j]);
        }
      }
...
/**
 * Traslates JS object to stylus hash.
 *
 * @param  {object} object  - JS object
 * @return {hash}           - Stylus hash
 */
var objectToHash = module.exports.objectToHash = function(object) {
  var hash = new n.Object();
  ...
  for(var i in object) {
    ...
    if(typeof item.nodeName == 'undefined') {
      if(!Array.isArray(item) && typeof item == 'object') {
        item = objectToHash(item);
      }
    }
